...
export const prepareExportContext = async (
  ...
  // AGGRESSIVE OPTIMIZATION: Rasterize Static Device Content
  // If we have a device animation wrapper, the content INSIDE it (the mockup) is usually static geometry.
  // We can render that geometry ONCE to an image, and then just animate the wrapper containing the image.
  // This avoids serializing the complex SVG mockup paths every frame.
  const deviceAnimWrapper = deviceClone.querySelector('[data-device-animation]') as HTMLElement;
  if (deviceAnimWrapper) {
      console.log('[Export] Rasterizing device content for performance...');
      
      // 1. Clone the wrapper's content to a temp container for capturing
      // We need to capture it at high resolution (output resolution)
      // The wrapper usually fills the container or has specific size.
      // We'll capture the *children* of the wrapper.
      const tempContainer = document.createElement('div');
      tempContainer.setAttribute('xmlns', 'http://www.w3.org/1999/xhtml');
      tempContainer.style.width = `${sourceWidth}px`;
      tempContainer.style.height = `${sourceHeight}px`;
      tempContainer.style.position = 'absolute';
      tempContainer.style.top = '0';
      tempContainer.style.left = '0';
      // Copy children
      while (deviceAnimWrapper.firstChild) {
          tempContainer.appendChild(deviceAnimWrapper.firstChild); 
          // Move children to temp container (removing from clone temporarily)
      }

      // 2. Render temp container to image
      // We use the same sync render logic but for this static part
      const serializer = new XMLSerializer();
      const tempSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${outputWidth}" height="${outputHeight}">
      ...
      ${serializer.serializeToString(tempContainer)}
      </div></foreignObject></svg>`;
      
      const tempImg = new Image();
      tempImg.crossOrigin = 'anonymous';
      const tempUrl = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(tempSvg)}`;
      
      await new Promise<void>((resolve, reject) => {
          tempImg.onload = () => resolve();
          tempImg.onerror = () => reject(new Error('Failed to rasterize device cache'));
          tempImg.src = tempUrl;
      });

      // 3. Draw to canvas to get a clean PNG (avoiding SVG nesting issues and getting pure pixels)
      const tempCanvas = document.createElement('canvas');
      tempCanvas.width = outputWidth;
      tempCanvas.height = outputHeight;
      const ctx = tempCanvas.getContext('2d')!;
      ctx.drawImage(tempImg, 0, 0);
      const deviceLookDataUrl = tempCanvas.toDataURL('image/png'); // Fast enough for one-time
      
      // 4. Replace content in deviceAnimWrapper with the rasterized image
      // Restore wrapper structure but with image child
      deviceAnimWrapper.innerHTML = ''; // Start clean (though we moved children out)
      const cachedImg = document.createElement('img');
      cachedImg.src = deviceLookDataUrl;
      cachedImg.style.width = '100%';
      cachedImg.style.height = '100%';
      // The image is already scaled to output size, but we are placing it inside the scaled source container.
      // Wait, our SVG export wrapper applies `scale(scaleX, scaleY)` to the root div.
      // The image we generated is size `outputWidth x outputHeight`.
      // If we put it back in `deviceAnimWrapper` (which is `sourceWidth x sourceHeight`), 
      // and then it gets scaled up... we might have double scaling or size mismatch.
      
      // Correction: We rasterized the *scaled* output.
      // If we put it back, we should unset the transform on the wrapper?
      // No, the wrapper has the ANIMATION transforms (rotate, scale). We need those.
      
      // Better approach:
      // Rasterize the content at `sourceWidth x sourceHeight` (unscaled).
      // Then let the main export pipeline handle the scaling up.
      // This preserves the coordinate system for the transforms on the wrapper.
      
      // --- RETRY STRATEGY: Source-Resolution Rasterization ---
      // Re-use logic but with source dimensions.
      const unscaledSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${sourceWidth}" height="${sourceHeight}">
      <foreignObject width="${sourceWidth}" height="${sourceHeight}">
      <div xmlns="http://www.w3.org/1999/xhtml" style="width:${sourceWidth}px;height:${sourceHeight}px;">
      ${serializer.serializeToString(tempContainer)}
      </div></foreignObject></svg>`;
      
      const unscaledImg = new Image();
      unscaledImg.crossOrigin = 'anonymous';
      const unscaledUrl = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(unscaledSvg)}`;
      await new Promise<void>((resolve, reject) => {
          unscaledImg.onload = () => resolve();
          unscaledImg.onerror = (e) => reject(e);
          unscaledImg.src = unscaledUrl;
      });

      const unscaledCanvas = document.createElement('canvas');
      unscaledCanvas.width = sourceWidth;
      unscaledCanvas.height = sourceHeight;
      const uCtx = unscaledCanvas.getContext('2d')!;
      uCtx.drawImage(unscaledImg, 0, 0);
      const unscaledDataUrl = unscaledCanvas.toDataURL('image/png');
      
      cachedImg.src = unscaledDataUrl;
      // Now it fits perfectly in the wrapper.
      deviceAnimWrapper.appendChild(cachedImg);
      
      console.log('[Export] Device content rasterized.');
  }
