...
const EXPORT_OPTIONS: ExportOption[] = [
  ...
  { id: 'webp', label: 'WebP Image', description: 'Smaller, transparent', icon: <ImageIcon className="w-4 h-4" /> },
];
...
export const Header = ({ onRender }: HeaderProps) => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const unzip = Command.create('unzip', [
        '-o', 
        filePath, 
        '-d', 
        extractPath
      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-bold tracking-tight uppercase text-white flex items-center gap-2">
            Liike <span className="text-[10px] text-ui-muted font-normal bg-ui-highlight/20 px-1.5 py-0.5 rounded-md">v{appVersion}</span>
          </h1>
          {updateAvailable && (
             <button 
