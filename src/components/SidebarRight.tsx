    ...
    <aside className="flex w-80 flex-col border-l border-ui-border bg-ui-bg-secondary h-full overflow-hidden">
      ...
      <div className="flex border-b border-ui-border">
        <button
          ...
          className={`flex-1 py-3 text-[11px] font-bold uppercase tracking-widest transition-colors ${activeTab === 'presets' ? 'text-white border-b-2 border-accent bg-ui-panel' : 'text-ui-muted hover:text-ui-text'}`}
        >
          Presets
        </button>
        <button
          onClick={() => setActiveTab('camera')}
          className={`flex-1 py-3 text-[11px] font-bold uppercase tracking-widest transition-colors ${activeTab === 'camera' ? 'text-white border-b-2 border-accent bg-ui-panel' : 'text-ui-muted hover:text-ui-text'}`}
        >
          Camera & Style
        </button>
      </div>
        ...
        {activeTab === 'camera' && (
          <div className="p-5 space-y-6">
            ...
            <div className="space-y-3">
              <h3 className="text-[10px] uppercase font-bold text-ui-muted tracking-widest">3D Rotation</h3>
              <div className="space-y-2">
                <label className="flex flex-col gap-1">
                  <span className="text-xs text-ui-text">Rotate X ({rotationX}°)</span>
                  <input
                    type="range" min="-60" max="60" value={rotationX}
                    onChange={(e) => setRotationX(Number(e.target.value))}
                    className="w-full accent-accent h-1 bg-ui-border rounded-lg appearance-none cursor-pointer"
                  />
                </label>
                <label className="flex flex-col gap-1">
                  <span className="text-xs text-ui-text">Rotate Y ({rotationY}°)</span>
                  <input
                ...
                <label className="flex flex-col gap-1">
                  <span className="text-xs text-ui-text">Rotate Z ({rotationZ}°)</span>
                  <input
            ...
            {/* Appearance Controls */}
            <div className="space-y-3">
              <h3 className="text-[10px] uppercase font-bold text-ui-muted tracking-widest">Appearance</h3>

              <label className="flex flex-col gap-1">
                <span className="text-xs text-ui-text">Shadow Opacity ({shadowOpacity}%)</span>
                <input
              ...
              <label className="flex flex-col gap-1">
                <span className="text-xs text-ui-text">Device Scale ({deviceScale.toFixed(2)}x)</span>
                <input
              ...
              <label className="flex flex-col gap-1">
                <span className="text-xs text-ui-text">Device Roundness ({cornerRadius}px)</span>
                <input
